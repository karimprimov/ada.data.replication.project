...
```{r}
#use species richness as the response variable, including all variables
boost.sp.rich.high.cont.tree <- gbm(formula = SR~+Dcoast_km+DCoralT_km+Depth+SST_median+SST_sd+DistSeamount_km, data = fb, n.trees = 100000)
...
```
A new BRT was run to attempt to replicate the exact same figure found in the publication (Figure 3A), however, the BRT in re-analysis using only variables found in Figure 3A in the publication was starkly different than Figure 3A in the publication. It is unclear why there is such a stark difference in plot outcome, both with respect to variable contribution as well as scale of contribution, however, it may be due to the fact that variables included in Figure 3A were transformed before usage in BRT analysis. 




Then, partial dependence plots were used to identify the relationship between distance to the coast and species richness, as well as the distance to the Coral Triangle and species, both with the removal of the influence of other contributing variables. Data for the partial dependence plots were used from the "boost.sp.rich.high.cont.tree", the BRT with reduced number of variables in an attempt to replicate Figure 3A in the publication. 
...
Overall, the partial dependence plot for identify the relationship between distance to the coast and species richness were similar in the fact that there is an overall negative trend between species richness and distance to coast, however, there appears to be an initial increase in the re-analyzed plot from 0- ~50 km from the coast, then a negative trend from 50- ~230 km, then an increase from ~230 - ~260 km offshore, then a decrease to ~300 km, then a plateau until ~330 km offshore. On the contrary, there's a gradual decline in the published figure from 0 - ~230, then a steep decline from ~230 - ~330 km offshore. It is unclear exactly why there's such a difference between the results in these two figures since re-analyzed data was un-transformed and plotted, however, the usage of the re-analyzed boosted regression tree, whose result is different from that in the publication, may contribute to the discrepancy between figure outcomes. 
