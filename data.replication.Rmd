...
fb <- f %>%
  mutate(Shark.Abundance = (expm1(fb$TaSharks)))
a <- ggplot(fb, aes(x = Shark.Abundance)) 
a + geom_density() 

summary(fb$Shark.Abundance)

#shark.abundance.density <- density(fb$Shark.Abundance, from = 0) 
...
boost.sp.rich.tree <- gbm(fb$SR~fb$Shark.Abundance+fb$MaxL+fb$Depth+fb$Chla_median+fb$Chla_sd+fb$SST_median+fb$SST_sd
                          +fb$pop_50km+fb$pop_500km+fb$Dist_nearest_pop+fb$Dist_nearest_market+fb$HDI+fb$Dcoast_km+
                            fb$DCoralT_km+fb$DistSeamount_km+fb$GDP+fb$Dist_SMT1500+fb$FishingEffort01+fb$FishingEffort05, data = fb)
...
shark.abund.boxplot <- boxplot(fb$TaSharks~fb$location)
shark.abund.boxplot
